"""Представления приложения новостей."""
...
class NewsDetail(generic.DetailView):
    ...
    def get_object(self, queryset=None):
        """Возвращает новость с заранее загруженными комментариями."""
    ...
    def get_context_data(self, **kwargs):
        """Добавляет форму комментария авторизованному пользователю."""
...
class NewsComment(
    ...
    """Обрабатывает создание комментария к новости."""
    ...
    def post(self, request, *args, **kwargs):
        """Сохраняет выбранную новость перед обработкой формы."""
        self.object = self.get_object()
        return super().post(request, *args, **kwargs)

    def form_valid(self, form):
        """Связывает комментарий с новостью и автором."""
    ...
    def get_success_url(self):
        """Возвращает адрес блока комментариев к новости."""
        post = self.get_object()
        return reverse('news:detail', kwargs={'pk': post.pk}) + '#comments'


class NewsDetailView(generic.View):
    """Распределяет GET- и POST-запросы к странице новости."""

    def get(self, request, *args, **kwargs):
        """Передаёт GET-запрос представлению новости."""
        view = NewsDetail.as_view()
        return view(request, *args, **kwargs)

    def post(self, request, *args, **kwargs):
        """Передаёт POST-запрос представлению создания комментария."""
